...
`
	...
	return generateFunctionsLambda(schema)
}

func generateFunctionsLambda(schema *proto.Schema) error {
...
func sstStacks(config *config.ProjectConfig, schema *proto.Schema) error {
	...
	err = t.Execute(&b, &StacksTemplateData{
		...
		HasFunctions:     node.HasFunctions(schema),
	})
	if err != nil {
...
func installNpmDeps(config *config.ProjectConfig, schema *proto.Schema) error {
	...
	if node.HasFunctions(schema) {
		files, err := node.Generate(context.Background(), schema)
		if err != nil {
			return err
		}
