...
type QueryBuilder struct {
	...
	Model *proto.Model
	// The underling schema for this model.
	schema *proto.Schema
...
func NewQuery(ctx context.Context, schema *proto.Schema, model *proto.Model, opts ...QueryBuilderOption) *QueryBuilder {
	qb := &QueryBuilder{
		context:    ctx,
		Model:      model,
		schema:     schema,
...
func (query *QueryBuilder) applyCursorFilter(cursor string, isBackwards bool) error {
	...
	for i := 0; i < len(query.orderBy); i++ {
		...
		for j := 0; j < i; j++ {
			...
			inline := NewQuery(query.context, query.schema, query.Model)
